...
> ColdReasoner 是这个面向智能体的形式化操作系统的形式化内核。所有其他组件都是可插拔的外挂装置，为其形式化一致性校验提供输入。系统的语义——信念区间、行为合法性、映射函数——均被形式化定义，使整个操作系统成为一个 **可数学验证的智能体运行时环境**。
...
这三条原则通过 **形式化操作系统抽象** 得以实现：智能体信念被类型化为区间，行为被类型化为带有形式化一致性规则的枚举，每一次决策都被记录为可回放、可对照形式规范验证的轨迹。这使得 ColdOS 有潜力成为一个 **完全可审计、可机器检查的智能体部署平台**。
